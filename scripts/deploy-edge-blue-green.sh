...
run_edge() {
  ...
  if [ "$http_port" = "80" ] && [ "$DATABASE_PROXY_PUBLIC_PORTS_EXPOSED" = "true" ]; then
    port_args+=(
      -p "${PXXL_DATABASE_PROXY_PUBLIC_BIND_HOST:-0.0.0.0}:${PXXL_DATABASE_PROXY_PUBLIC_PORT_RANGE:-25000-40000}:${PXXL_DATABASE_PROXY_PUBLIC_PORT_RANGE:-25000-40000}"
    )
  fi
